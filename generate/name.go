...
func GenerateRandomWord(wordLen int64) (string, error) {
...
func GenerateAvatarURL() (string, error) {
	randomWord, err := GenerateRandomWord(32)
	if err != nil {
		return "", fmt.Errorf("failed to generate random avatar url, %w", err)
	}
	return fmt.Sprintf("https://www.gravatar.com/avatar/%s?d=identicon", randomWord), nil
}
